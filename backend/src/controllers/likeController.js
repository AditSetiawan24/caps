...
const toggleLikeMovie = async (req, res) => {
  const { id_film, is_like } = req.body; // is_like bernilai true (like) atau false (dislike)
  ...
  try {
    // 1. Cek dulu apakah user sudah pernah ngasih reaksi di film ini sebelumnya
    ...
    if (checkExist.rows.length > 0 && checkExist.rows[0].is_like === is_like) {
      ...
      return res.status(200).json({
        success: true,
        reaksiAktif: null, // Frontend bisa pakai ini untuk matikan highlight tombol dua-duanya
        message: 'Reaksi kamu berhasil dihapus (Batal)!'
      });
    }

    // 3. LOGIKA SIMPAN/UBAH: Jika belum ada reaksi, atau beda pilihan (dari like ganti dislike), jalankan UPSERT
    await db.query(
    ...
    res.status(200).json({ 
      success: true, 
      reaksiAktif: is_like ? 'like' : 'dislike', // Memudahkan anak frontend buat nge-handle state CSS tombol
